...
export const userJWT = async (req: Request, res: Response, next) => {
    ...
    if (authorization) {
        try {
            ...
            // Check single device session token (only for role === 'user', admin can login on multiple devices)
            if (result?.role === 'user' && result?.currentToken && result.currentToken !== token) {
